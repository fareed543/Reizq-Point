...
export class TransactionComponent implements OnInit {
  ...
  onDescriptionInput(event: any) {
    ...
    const amountRegex = /INR\s*(\d+(?:\.\d{1,2})?)/;
    ...
    if (dateMatch) {
      ...
      const formattedYear = 2000 + year;
      formattedDate = `${formattedYear}-${String(month).padStart(2, "0")}-${String(day).padStart(2, "0")}`;
    } else {
    ...
    if (message.toLowerCase().includes('debited')) {
      type = 'expense';
    } else if (message.toLowerCase().includes('credited')) {
      type = 'income'; 
    } 
