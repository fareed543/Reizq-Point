...
export class AppComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    //     if(event.urlAfterRedirects === '/pagenotfound'){
    //       this.localStorageService.setItem('currentURL', '/dashboard');
    ...
    setTimeout(() => {
      if (window.self !== window.top) {
        this.renderer.addClass(document.body, 'iframe');
      } else {
        this.renderer.removeClass(document.body, 'iframe');
      }
    }, 0);
